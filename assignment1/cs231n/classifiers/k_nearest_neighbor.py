...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X: np.array):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            # X[i] is a vector of shape (D,)
            # self.X_train is a matrix of shape (num_train, D)
            # X[i] - self.X_train uses broadcasting to produce a matrix of shape (num_train, D)
    ...
    def predict_labels(self, dists: np.array, k: int = 1):
        ...
        for i in range(num_test):
            ...
            neighbours_labels = self.y_train[nearest_neighbors_indices]
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            y_pred[i] = np.argmax(np.bincount(neighbours_labels))
