...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # Step 1: Calculate the mean of the minibatch
        sample_mean = np.mean(x, axis=0)

        # Step 2: Calculate the variance of the minibatch
        ...
        # Step 6: Store intermediates for backpropagation
...
def batchnorm_backward(dout, cache):
    ...
    # Step 1: Compute gradients of gamma and beta
    dgamma = np.sum(dout * x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)

    # Step 2: Backprop into normalized x
    dx_hat = dout * gamma

    # Step 3: Backprop through normalization
    dsample_var = np.sum(dx_hat * (x - sample_mean) * -0.5 * (sample_var + eps) ** -1.5, axis=0)
    dsample_mean = np.sum(dx_hat * -1.0 / np.sqrt(sample_var + eps), axis=0) + dsample_var * np.mean(
        -2.0 * (x - sample_mean), axis=0)

    # Step 4: Backprop into input x
...
def batchnorm_backward_alt(dout, cache):
    ...
    # Step 2: Compute gradient with respect to the input x
    dx_hat = dout * gamma
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        # Apply dropout mask to input data
        out = x * mask
